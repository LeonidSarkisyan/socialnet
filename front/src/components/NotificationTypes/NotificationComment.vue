<template>
  <div class="div__wrapper" @click="openModelPost">
    <div class="div">
      <div class="wrapper">
        <div class="avatarS">
          <router-link :to="`/profile/${notification.comment.profile.tag_name}`" @click.stop="this.$emit('hideNotiList')">
            <div class="box">
        ...
        <div class="info">
          <router-link :to="`/profile/${notification.comment.profile.tag_name}`" class="people" @click.stop="this.$emit('hideNotiList')">
          <span class="people">
            {{ notification.comment.profile.name }} {{ notification.comment.profile.surname }}
          </span>
          </router-link>
          оставил комментарий:
          <div class="comment-text-noti">{{ limitText }}</div>
          <div class="date">
...
export default {
  ...
  methods: {
    openModelPost() {
      this.$emit('openModalPost', this.notification.comment.post.id)
    },
    fetchAvatar() {
  ...
  computed: {
    limitText() {
      if (this.notification.comment.text.length > 100) {
        return this.notification.comment.text.slice(0, 140) + '...'
      } else {
        return this.notification.comment.text
      }
    }
  },
  mounted() {
