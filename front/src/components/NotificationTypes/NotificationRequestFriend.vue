<template>
  <div class="div__wrapper">
    <div class="div">
      <div class="wrapper">
        <div class="avatarS">
          <router-link :to="`/profile/${notification.request_friend.profile_from.tag_name}`" @click.stop="this.$emit('hideNotiList')">
            <div class="box">
        ...
        <div class="info">
          <router-link :to="`/profile/${notification.request_friend.profile_from.tag_name}`" class="people" @click.stop="this.$emit('hideNotiList')">
          <span class="people">
            {{ notification.request_friend.profile_from.name }} {{ notification.request_friend.profile_from.surname }}
          </span>
          </router-link>
          ...
          <div class="divs" v-if="!leave">
            <div class="friend-button" @click="accept">
              Принять
            </div>
            <div class="friend-button" @click.stop="deleteNotification">
              Отклонить
            </div>
          </div>
          <div class="divs" v-else style="margin-top: 10px;">
            Вы приняли запрос
          </div>
...
export default {
  name: "NotificationRequestFriend",
  components: {
    ModalPost
  },
  props: {
  ...
  methods: {
    ...
    accept() {
      axios.post(`http://127.0.0.1:8000/requestsfriends/${this.notification.request_friend.id}/accept`).then(response => {
        console.log(response.data)
        this.leave = true
        this.$store.commit('updateFetchNotification', this.notification.id)
      }).catch(e => {
        this.leave = true
        this.$store.commit('updateFetchNotification', this.notification.id)
      })
    ...
    fetchAvatar() {
      fetch(`http://127.0.0.1:8000/profile/avatar/${this.notification.request_friend.profile_from.tag_name}`).then(response => {
    ...
    validatorLeave() {
      if (this.notification.request_friend.accept) {
        this.leave = true
      }
    }
  },
